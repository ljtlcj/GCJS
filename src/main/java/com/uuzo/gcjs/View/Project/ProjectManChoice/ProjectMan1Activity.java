...
public class ProjectMan1Activity extends BaseActivity {
    private ListView lv_listview;//展示列表
    private List<Member.ContentBean> lists = new ArrayList<Member.ContentBean>();//listview数据集
    private ProjectChoicean1AdapterM adapter;//适配器
    @Override
    public int getLayoutId() {
        return R.layout.activity_man_choice1;
    }
    ...
    private void init() {
        ...
        ProjectNewController.choice(params, parts, new InterfaceManger.OnRequestListener() {
            ...
            public void onSuccess(Object success) {
                ...
                for (int i = 0;i<workTaskProjectMessage.getContent().size();i++){
                    if(workTaskProjectMessage.getContent().get(i).getJob().equals("工程部经理")){
                        lists.add(workTaskProjectMessage.getContent().get(i));
                    }
                }
                adapter = new ProjectChoicean1AdapterM(ProjectMan1Activity.this,lists);
