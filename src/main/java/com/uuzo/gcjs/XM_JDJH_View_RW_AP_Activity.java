...
public class XM_JDJH_View_RW_AP_Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState)
	{
	    ...
	    setContentView(R.layout.activity_xm_jdjh_view_rw_ap);
        ...
        CM_UserID=getIntent().getIntExtra("CM_UserID",0);
        CM_UserName=getIntent().getStringExtra("CM_UserName");
        CM_Tel=getIntent().getStringExtra("CM_Tel");
        SO_UserID=getIntent().getIntExtra("SO_UserID",0);
        SO_UserName=getIntent().getStringExtra("SO_UserName");
        SO_Tel=getIntent().getStringExtra("SO_Tel");
        QC_UserID=getIntent().getIntExtra("QC_UserID",0);
        QC_UserName=getIntent().getStringExtra("QC_UserName");
        QC_Tel=getIntent().getStringExtra("QC_Tel");

	    if (Config.NowCls==null || CM_UserID<=0 || SO_UserID<=0 || QC_UserID<=0)
		{
	    	finish();
			return;
		}
        ...
        ((TextView)widget_3.getChildAt(1)).setText(CM_UserName);
        ((TextView)widget_3.getChildAt(3)).setText(SO_UserName);
        ((TextView)widget_3.getChildAt(5)).setText(QC_UserName);
        ...
        widget_0.setOnClickListener(new OnClickListener()
        {
            ...
            public void onClick(View v) {
                ...
                ParmStr+="&cm="+CM_UserID;
                ParmStr+="&so="+SO_UserID;
                ParmStr+="&qc="+QC_UserID;
                if (Value_2.equals("")){
			...
			if (FunName.equals("set")) {
                try {
                    ...
                    if (_JSONObject.getBoolean("status")) {
                        ...
                        _Cls.Int_9=2;
                        _Cls.Str_6=((EditText)widget_1.getChildAt(1)).getText().toString().trim();
                        _Cls.Str_12=CM_UserName;
                        _Cls.Str_13=SO_UserName;
                        _Cls.Str_14=QC_UserName;
